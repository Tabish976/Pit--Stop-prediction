...
│
├── pipeline_cache/            # Generated directory for caching intermediate artifacts
│   ├── config.json            # Saved pipeline configuration (e.g., USE_GPU, EARLY_STOP)
│   ├── col_names.json         # Saved list of final feature column names
│   ├── X.npy                  # Engineered and encoded training features (NumPy array)
│   ├── y.npy                  # Target variable array ('PitNextLap')
│   ├── X_test.npy             # Engineered and encoded testing features (NumPy array)
│   ├── tune_idx.npy           # Row indices used for the 20% hyperparameter tuning sample
│   └── lgb_tuning_result.pkl  # Pickled results and best parameters from LightGBM tuning
│
└── submission.csv             # Generated predictions ready for Kaggle submission 2
```
